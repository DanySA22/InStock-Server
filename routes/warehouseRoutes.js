  ...
  .post(async (req, res) => {
    try {
      ...
      if (
        ...
        String(req.body.contact_phone).length !== 10 ||
...
router.route("/:id").put(warehouseValidator, async (req, res) => {
  ...
  try {
    // Execute the update operation without .returning() method
    await knex("warehouses").where({ id }).update(updateData);
